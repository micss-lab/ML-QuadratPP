...
class ThingMLScopeProvider extends AbstractThingMLScopeProvider {
	...
	override getScope(EObject context, EReference reference) {
		...
		else if (reference == p.dataAnalytics_Input_features) { // For input features
            return scopeForDataAnalyticsInputFeatures(context as DataAnalytics);
        }
        else if (reference == p.dataAnalytics_Output_features) { // For output features
            return scopeForDataAnalyticsOutputFeatures(context as DataAnalytics);
        }
		else {
	...
	//input
	def  protected IScope scopeForDataAnalyticsInputFeatures(DataAnalytics context) {
         Scopes.scopeFor(ThingMLHelpers.allProperties(ThingMLHelpers.findContainingThing(context)));
    }

    // Add scope methods for output features
   def  protected IScope scopeForDataAnalyticsOutputFeatures(DataAnalytics context) {
         Scopes.scopeFor(ThingMLHelpers.allProperties(ThingMLHelpers.findContainingThing(context)));
    }
